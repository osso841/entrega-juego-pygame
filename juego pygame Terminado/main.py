...
while running:
    ...
    if comenzar_juego:
        ...
        if ingreso == palabra:
            ...
            if palabras_correctas < 10:
                lista_palabra = palabras_4_5_letras_sin_tilde
            elif palabras_correctas < 20:
                lista_palabra = palabras_6_7_letras_sin_tilde
            else:
                lista_palabra = palabras_7_8_letras_sin_tilde
            palabra = random.choice(lista_palabra)
            lista_palabra.remove(palabra)
        ...
        #NOTE: ACCIONES POR CANTIDAD DE PALABRAS ACERTADAS:
        if(primera_etapa and palabras_correctas == 10) or (segunda_etapa and palabras_correctas == 20):
            vidas += 1
            sonido_vida.play()
            if primera_etapa:
                primera_etapa = False
                ...
                segunda_etapa = False
        elif palabras_correctas == 30:
            ingresar_menu_tiempo = True
        ...
        if not personaje.Rect.colliderect(enemigo.Rect):
            if mapa_derecha.Rect.colliderect(enemigo.Rect):#enemigo por derecha
                ...
                if palabras_correctas < 10:
                    ...
                    enemigo.cambiar_imagen('imagenes\\movimiento_enemigo_izquierda1_1.png', 11, 1747, 178)
                elif palabras_correctas < 30:
                    enemigo.cambiar_imagen('imagenes\\movimiento_enemigo_izquierda1_2.png', 11, 1747, 178)
                ...
                if palabras_correctas < 20:
                    if tiempo_incremento_velocidad_enemigo > 10:
                        enemigo.posicion_x -= 8
                else:
                    if tiempo_incremento_velocidad_enemigo > tiempo_random:
                        enemigo.posicion_x -= 8

                if palabras_correctas < 10:
                    enemigo.posicion_x -= 5
                elif palabras_correctas < 20:
                    enemigo.posicion_x -= 7
                else:
                    enemigo.posicion_x -= 8
                ...
                if palabras_correctas < 10:
                    ...
                    enemigo.cambiar_imagen('imagenes\\movimiento_enemigo_derecha_1.png', 11, 1747, 178)
                elif palabras_correctas < 30:
                    enemigo.cambiar_imagen('imagenes\\movimiento_enemigo_derecha_2.png', 11, 1747, 178)
        ...
        if not mapa_derecha.Rect.contains(enemigo.Rect) and not mapa_izquierda.Rect.contains(enemigo.Rect):
            if not(mapa_derecha.Rect.colliderect(enemigo.Rect) and mapa_izquierda.Rect.colliderect(enemigo.Rect)):
                if presionar_tecla[K_9] and not presionar_tecla[K_3]:
                    enemigo.posicion_x += 10
                elif presionar_tecla[K_3] and not presionar_tecla[K_9]:
                    enemigo.posicion_x -= 10
    ...
    if comenzar_juego:
        ...
        vidas_juego.posicion_x = posicion_vida
        for e_vida in range(vidas):
            vidas_juego.dibujar(screen, vidas_juego.posicion_x)
            vidas_juego.posicion_x += 70
